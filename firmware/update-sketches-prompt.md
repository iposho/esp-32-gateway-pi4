...
Типы команд и как их показывает админка:

| `type` | Интерфейс | Payload команды |
|---|---|---|
| `toggle` | переключатель вкл/выкл; состояние берётся из поля телеметрии с именем `action` | `{"action":"led","value":true}` |
| `range` | ползунок; поля `min`, `max`, `step`, `unit` (по умолчанию 0…100, шаг 1) | `{"action":"brightness","value":70}` |
| `trigger` | кнопка «Выполнить» | `{"action":"capture"}` |

Команды `reboot`, `restart`, `reset`, `format`, `erase`, `ota` считаются опасными:
админка убирает их из основного управления в раздел «Обслуживание» и спрашивает
подтверждение. Чтобы админка подтверждала выполнение («Готово»), после каждой
команды сразу публикуйте телеметрию с новым состоянием.
